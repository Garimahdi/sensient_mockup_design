...
// Set logic for character exception
let letterNo = $('#no_surat');
letterNo.keyup(function(event){
    if(event.key == '/'){
        letterNo.val('-');
    }
});
...
// Auto add dropdown
let idx = Array.from({length:101},(v,k)=>k+1);
console.log(idx);

for(i in idx){
    document.getElementById('indexing').innerHTML += '<option value ="'+i+'">'+i+'</option>'
};